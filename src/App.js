...
class Search extends React.Component {
  ...
  handleAnimeSelected = (malId) => {
    ...
    fetch("http://localhost:10000/anime/" + malId + "/mainCharacters")
      ...
      .then(
        (result) => {
          console.log("voice actors: " + JSON.stringify(result))
          this.setState({
            mainCharacters: result
          });
        },
        // Note: it's important to handle errors here
        // instead of a catch() block so that we don't swallow
  ...
  render() {
    const { animes, selectedAnime, mainCharacters, error } = this.state;
    ...
    console.log('I am rendering with the state mainCharacters.length: ' + this.state.mainCharacters.length);
...
function MainCharacter(props) {
  console.log('props.voiceActors[0]: ' + props.voiceActors[0]);
  console.log('props.voiceActors[0].image_url: ' + props.voiceActors[0].image_url);
  return (
    <Flex container margin="10px auto" justifyContent="space-around" flexWrap="wrap">
      <Flex container flexDirection="column" alignItems="flex-start" alignItems="center"  padding="50px">
            <h1>{props.name}</h1>
            <a href={props.url} target="_blank" rel="noopener noreferrer"><img src={props.imageUrl} alt=""></img></a>
      </Flex>
      <Flex container flexDirection="column" alignItems="flex-start" alignItems="center"  padding="50px">
            <h1>{props.voiceActors[0].name}</h1>
            <a href={props.voiceActors[0].url} target="_blank" rel="noopener noreferrer"><img src={props.voiceActors[0].image_url} alt=""></img></a>
      </Flex>
      </Flex>
  )
}
