...
class Search extends React.Component {
  ...
  getHTML2 = (index) => {
    ...
    if ((mainCharacters !== undefined && persons !== undefined) && (mainCharacters.length == persons.length)) {
      ...
      var i;
      for (i = 0; i < mainCharacters.length; i++) {
          let character = mainCharacters[i];
  ...
  render() {
    const { animes, selectedAnimes, mainCharacters, persons, error, showComparison } = this.state;
    ...
    if (error) {
      ...
      return (
        ...
        <div>
          ...
          <input type="submit" value="show comparison" onClick={this.showComparison} />
          <br></br>
          <SearchBar
          ...
          {/* <Flex container flexDirection="row" margin="10px auto" flexWrap="wrap">
            {this.getHTML(mainCharacters, persons, 123)}
          </Flex> */}
